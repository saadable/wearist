      ...
      <div className='grid grid-cols-1 lg:grid-cols-3 gap-3 md:gap-8'>
        ...
        <div className='lg:col-span-2 space-y-3'>
          {items.map(item => (
            <div
              key={item.id}
              className='flex items-center gap-3 p-3 bg-white border border-[#2785ca] rounded-lg shadow-sm hover:shadow-md transition-shadow duration-200 h-[90px]'
            >
              {/* Product Image - Left Side */}
              <div className='shrink-0 w-16 h-16 rounded-md overflow-hidden border border-gray-200 bg-gray-50'>
                <Image
              ...
              <div className='flex-1 flex flex-col justify-center min-w-0'>
                <h2 className='text-sm font-semibold text-[#2785ca] line-clamp-1'>{item.title}</h2>
                <p className='text-xs text-gray-600 mt-1'>PKR {item.new_price}</p>
                ...
                <div className='flex items-center justify-between gap-2 mt-2'>
                  <div className='flex items-center gap-1.5 bg-gray-100 rounded-md p-1'>
                    <button
                      ...
                      className='p-1 text-[#2785ca] hover:bg-[#2785ca] hover:text-white rounded transition-colors'
                      aria-label='Decrease quantity'
                    >
                      <FaMinus size={12} />
                    </button>
                    <span className='px-2 py-0.5 font-semibold text-center min-w-8 text-xs text-[#2785ca]'>{item.quantity}</span>
                    <button
                  ...
                  <span className='text-xs font-bold text-green-600'>PKR {item.new_price * item.quantity}</span>
